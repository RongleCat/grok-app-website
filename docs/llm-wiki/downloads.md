...
| 现网（2026-09-10 核对） | 构建已拉到 `downloads.json` tag `v0.2.34`（产品仓正式 Release，2026-09-09）。稳定安装包名仍挂在 latest。若下次 404，脚本改走 latest Release tag，按钮仍走写死的稳定 URL |
...
- [x] 若构建时拉了清单或 latest Release tag，页面版本号与现网正式 tag 一致（本机构建见 `v0.2.34`）
