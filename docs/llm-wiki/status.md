...
| Hero / 开源页 GitHub star | 已实现 | 首页 `hero.ctaGithub` 与 `/opensource/` `oss.repo.cta` 旁显示缩写；悬停/聚焦精确数；首屏画 `src/generated/stars-meta.json`；每次进页拉公开 API；失败藏数字只留原按钮。无 `grok-app-site.stars` / 1h TTL。契约 [stars.md](./stars.md) |
