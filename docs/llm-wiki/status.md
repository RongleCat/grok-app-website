...
最后更新：2026-08-30。本页必须与仓库事实一致。

## 一句话

官网已上线。推 `main` 走 GitHub Actions 构建并部署 Cloudflare Pages。搜索意图覆盖 Grok Desktop / Grok GUI / 桌面客户端，品牌短称「开源 Grok App」。站点不再写非官方免责声明。SEO/GEO 基础（www→apex 301、sitemap、`llms.txt`、JSON-LD、`/faq/`、`/install/`）已在。社区皮肤画廊在 `/skins/`：等大 3–4 列（手机 2 列）、剪影跟 `html[data-theme]` 走、桌面 Apply 发 `grok://`、手机 toast。`/opensource/` 贡献者墙现为 8 人。聊天里出现过的 CF token 视为已暴露，建议 Dashboard 轮换。
...
| 项 | 状态 | 证据 |
|----|------|------|
| 本仓代码 | 已落地多页 | `index.html` + `opensource/` + `faq/` + `skins/` + `install/` + `src/` + `public/` |
...
| `downloads.json` 现网 | 构建时可拉到 `v0.2.26`；失败回退稳定 URL | `src/generated/downloads-meta.json` + `scripts/fetch-downloads.mjs` |
...
| Hero / 开源页 GitHub star | 已实现 | 首页 `hero.ctaGithub` 与 `/opensource/` `oss.repo.cta` 旁显示缩写；悬停/聚焦精确数；首屏画 `src/generated/stars-meta.json`；运行时浏览器只打同源 `/api/stars`（`functions/api/stars.ts` 服务端拉 GitHub）；失败保留构建回退，不藏数字。无 `grok-app-site.stars` / 1h TTL；不打 `api.github.com`。契约 [stars.md](./stars.md) |
...
| sitemap | 已实现 | `public/sitemap.xml` 含 `/` `/opensource/` `/faq/` `/skins/` `/install/`；安装页 `lastmod` 2026-08-26 |
| llms.txt | 已实现 | `public/llms.txt`；Also known as + 产品名 Grok App；画廊 URL 为本站 `/skins/`；安装指南 `/install/` |
| JSON-LD / meta | 已实现 | 首页 SoftwareApplication + Organization + WebSite；皮肤页 CollectionPage；安装页 HowTo；`alternateName`；`softwareVersion` 跟 `downloads-meta.json`（`v0.2.26`）；短 title；`twitter:site` `@cgnot996` |
