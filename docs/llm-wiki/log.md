...
- **操作者**：agent
- **触发**：最新非 draft Release 已是 v0.2.36（2026-09-16）；现网 JSON-LD / downloads-meta / changelog 顶条仍写 v0.2.35
- **改动**：
  - `node scripts/fetch-downloads.mjs` 拉现网 `downloads.json`，`src/generated/downloads-meta.json` tag → `v0.2.36`，同步首页 JSON-LD `softwareVersion`
  - `/changelog/` 置顶 v0.2.36（三语摘要取自产品仓 Release notes：置顶对话、复制命令和 diff、共享会话数据记住）；旧条目保留；ItemList `numberOfItems` 11
  - `public/llms.txt` 当前稳定版 → `v0.2.36`
  - `public/sitemap.xml` `/` `/install/` `/changelog/` lastmod → 2026-09-17
  - 未改 star 数代码，未发明 Search Console 数据
- **Wiki**：downloads / seo / content / design / status / 本条
- **结果**：下载旁注、安装页 `data-version`、JSON-LD、`llms.txt` 与 changelog 顶条同为 `v0.2.36`
- **未做 / 下一步**：短链 `/download/*` 仍可选
...
- **操作者**：agent
- **触发**：最新非 draft Release 已是 v0.2.34（2026-09-09）；现网 JSON-LD / downloads-meta / changelog 顶条仍写 v0.2.33
- **改动**：
  - `node scripts/fetch-downloads.mjs` 拉现网 `downloads.json`，`src/generated/downloads-meta.json` tag → `v0.2.34`，同步首页 JSON-LD `softwareVersion`
  - `/changelog/` 置顶 v0.2.34（三语摘要取自产品仓 Release notes：壁纸来源扩展、Windows 卡死修复、聊天缩略图拦内网、Ctrl+Tab / Mermaid）；旧条目保留；ItemList `numberOfItems` 9
  - `public/llms.txt` 当前稳定版 → `v0.2.34`
  - `public/sitemap.xml` `/` `/install/` `/changelog/` lastmod → 2026-09-10
  - 未改 star 数代码，未发明 Search Console 数据
- **Wiki**：downloads / seo / content / design / status / 本条
- **结果**：下载旁注、安装页 `data-version`、JSON-LD、`llms.txt` 与 changelog 顶条同为 `v0.2.34`
- **未做 / 下一步**：短链 `/download/*` 仍可选
...
- **操作者**：agent
- **触发**：最新非 draft Release 已是 v0.2.33（2026-09-06）；现网 JSON-LD / downloads-meta / changelog 顶条仍写 v0.2.32
- **改动**：
  - `node scripts/fetch-downloads.mjs` 拉现网 `downloads.json`，`src/generated/downloads-meta.json` tag → `v0.2.33`，同步首页 JSON-LD
  - `/changelog/` 置顶 v0.2.33（三语摘要取自产品仓 Release notes：Windows 安装包回挂、输入框快捷键、worktree prune、发版门禁）；旧条目保留
  - `public/sitemap.xml` `/` `/install/` `lastmod` → 2026-09-07；`/changelog/` 仍为 2026-09-02（测试锁该日）
  - 未改 star 数代码，未发明 Search Console 数据
- **Wiki**：downloads / seo / content / design / status / 本条
- **结果**：下载旁注、安装页 `data-version`、JSON-LD 与 changelog 顶条同为 `v0.2.33`；稳定安装包名已回挂
- **未做 / 下一步**：产品仓 `repository_dispatch` 仍未做
...
- **操作者**：agent
- **触发**：最新非 draft Release 已是 v0.2.31（2026-09-04）；现网 JSON-LD / downloads-meta 仍写 v0.2.30
- **改动**：
...
- **操作者**：agent
- **触发**：最新非 draft Release 已是 v0.2.29（2026-08-30）；现网 JSON-LD 仍写 v0.2.26
- **改动**：
  - `node scripts/fetch-downloads.mjs` 拉现网 `downloads.json`，`src/generated/downloads-meta.json` tag → `v0.2.29`
  - 脚本同步首页 JSON-LD `SoftwareApplication.softwareVersion` → `v0.2.29`（不再只手改 HTML）
  - `public/sitemap.xml` 首页与 `/install/` `lastmod` → 2026-08-31（版本文案变了）
  - 未改 star 数代码，未发明 Search Console 数据
- **Wiki**：downloads / seo / status / 本条
- **结果**：下载旁注、安装页 `data-version`、JSON-LD 与产品仓 latest Release 同为 `v0.2.29`
- **未做 / 下一步**：产品仓 `repository_dispatch` 仍未做
