...
- 当前稳定版与 `downloads-meta.json` 的 `tag` 一致（现为 `v0.2.36`），并指向 `https://grok-app.com/changelog/`
...
- `softwareVersion` 必须等于 `src/generated/downloads-meta.json` 的 `tag`（当前 `v0.2.36`）。`fallback: true` 时不要写版本
